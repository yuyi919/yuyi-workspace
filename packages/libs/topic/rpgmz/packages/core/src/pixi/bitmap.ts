...
export class Bitmap {
  ...
  _createBaseTexture(
    source: HTMLCanvasElement | HTMLImageElement | HTMLVideoElement | PIXI.resources.Resource
  ): void {
  ...
  _startLoading(): void {
    ...
    //@ts-ignore
    (window.a || (window.a = [])).push(this._url);
    // console.log(Utils.hasEncryptedImages())
    if (Utils.hasEncryptedImages()) {
      this._startDecrypting();
    } else {
      this._image.src = unescape(this._url);
    }
  }
