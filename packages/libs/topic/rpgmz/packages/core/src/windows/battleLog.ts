...
import { Game_Action, Game_Actor, Game_Battler, Game_Enemy } from "../game";
import { $dataAnimations, $gameTemp, DataManager, SoundManager, TextManager } from "../managers";
import { MZ } from "../MZ";
...
export class Window_BattleLog extends Window_Base {
  ...
  push(methodName: string, ...params: any): void {
    this._methods.push({ name: methodName, params: params });
  }
  ...
  displayCounter(target: Game_Battler): void {
    ...
    this.push(
      "addText",
      TextManager.counterAttack.format((target as Game_Actor | Game_Enemy).name())
    );
  }
  ...
  displayReflection(target: Game_Battler): void {
    ...
    this.push(
      "addText",
      TextManager.magicReflection.format((target as Game_Actor | Game_Enemy).name())
    );
  }
  ...
  displaySubstitute(substitute: Game_Battler, target: Game_Battler): void {
    ...
    const text = TextManager.substitute.format(
      substName,
      (target as Game_Actor | Game_Enemy).name()
    );
  ...
  displayFailure(target: Game_Battler): void {
    if (target.result().isHit() && !target.result().success) {
      this.push(
        "addText",
        TextManager.actionFailure.format((target as Game_Actor | Game_Enemy).name())
      );
    }
  }
