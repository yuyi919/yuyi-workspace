...
ApiAdapter*
NewAdapter(const string&, const string&, float, float, float, float, int32_t, int32_t, int32_t);
...
Ebyroid* Ebyroid::Create(const string& base_dir,
                         const string& voice,
                         float volume,
                         float speed,
                         float pitch,
  ...
  ApiAdapter* adapter = NewAdapter(
      base_dir, voice, volume, speed, pitch, range, pause_long, pause_middle, pause_sentence);
  Dprintf("Ebyroid::Create NewAdapter", );
...
int Ebyroid::Speech(const unsigned char* inbytes,
  ...
  // std::string kana;
  // int32_t job_id2;
  // api_adapter_->TextToKana2(&job_id2, &param, (const char*) inbytes, &kana);
...
int Ebyroid::Convert(const ConvertParams& params,
  ...
  if (params.needs_reload) {
    ...
    Dprintf(
        ...
        params.pause_long,
        params.pause_middle,
        params.pause_sentence);
    api_adapter_ = NewAdapter(params.base_dir,
                              params.voice,
                              params.volume,
                              params.speed,
                              params.pitch,
...
ApiAdapter* NewAdapter(const string& base_dir,
                       const string& voice,
                       float volume,
                       float speed,
                       float pitch,
  ...
  if (ResultCode result = adapter->Init(&config); result != ERR_SUCCESS) {
    ...
    Eprintf("%s", message.c_str());
    throw std::runtime_error(message);
  }

  Dprintf("Ebyroid::Create\nvolume:%f\nspeed:%f\npitch:%f\nrange:%f\npause_long:%d\npause_middle:%"
          "d\npause_"
          "sentence:%d",
          volume,
          speed,
  ...
  auto [is_error, what] = WithDirecory(settings.base_dir, [adapter, settings]() {
    if (ResultCode result = adapter->LangLoad(settings.language_dir); result != ERR_SUCCESS) {
      char m[64];
      Eprintf("API LangLoad failed (could not load language) with code %d", result);
