  ...
  *(buffer + node_buffer_size) = '\0';
  ...
  if (is_param) {
    ...
    if (params->needs_reload) {
      ...
      printf("volume%f", *(double *)value);
      en_assert(status == napi_ok);
      status = napi_get_named_property(env, argv[1], "speed", &value);
      en_assert(status == napi_ok);
      printf("speed%f", *(double *)value);
      ...
      en_assert(status == napi_ok);
      status = napi_get_named_property(env, argv[1], "pause_long", &value);
      en_assert(status == napi_ok);
      status = napi_get_value_int32(env, value, &pause_long);
      en_assert(status == napi_ok);
      status = napi_get_named_property(env, argv[1], "pause_middle", &value);
      en_assert(status == napi_ok);
      status = napi_get_value_int32(env, value, &pause_middle);
      en_assert(status == napi_ok);
      status = napi_get_named_property(env, argv[1], "pause_sentence", &value);
      en_assert(status == napi_ok);
      status = napi_get_value_int32(env, value, &pause_sentence);
      en_assert(status == napi_ok);
      params->volume = (float) volume;
      params->speed = (float) speed;
      params->pitch = (float) pitch;
      params->range = (float) range;
      params->pause_middle = pause_middle;
      params->pause_sentence = pause_sentence;
      params->pause_long = pause_long;
...
//
// JS Signature: init(baseDir: string, voice: string, volume: number, speed: number, pitch: number,
// range: number, pause_long: number, pause_middle: number, pause_sentence: number,) -> none
//
static napi_value export_func_init(napi_env env, napi_callback_info info) {
  ...
  en_assert(status == napi_ok);
  status = napi_get_value_double(env, argv[3], &speed);
  en_assert(status == napi_ok);
  status = napi_get_value_double(env, argv[4], &pitch);
  en_assert(status == napi_ok);
  status = napi_get_value_double(env, argv[5], &range);
  en_assert(status == napi_ok);
  status = napi_get_value_int32(env, argv[6], &pause_long);
  en_assert(status == napi_ok);
  status = napi_get_value_int32(env, argv[7], &pause_middle);
  en_assert(status == napi_ok);
  status = napi_get_value_int32(env, argv[8], &pause_sentence);
  en_assert(status == napi_ok);

  Dprintf("initialize ebyroid");
  // initialize ebyroid
  try {
    module->ebyroid = Ebyroid::Create(install_dir_buffer,
                                      voice_dir_buffer,
                                      (float) volume,
                                      (float) speed,
                                      (float) pitch,
  ...
  status = napi_add_env_cleanup_hook(
      env, [](void* arg) { delete module->ebyroid; }, NULL);
  en_assert(status == napi_ok);
