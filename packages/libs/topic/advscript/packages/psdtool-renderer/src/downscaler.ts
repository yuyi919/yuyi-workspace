import { scale } from "downscaler/index";
import rawDownscaler from "downscaler/downscaler.js?raw";
export class DownScaler {
  get destWidth(): number {
    return 0 | Math.max(1, this.src.width * this.scale);
  }
  get destHeight(): number {
    return 0 | Math.max(1, this.src.height * this.scale);
  }
  ...
  public scaleWith(g?: number) {
    if (typeof g !== "number") return this.fast();
    this.adjustSize();
    ...
    scale(srcImageData, destImageData, g);
  ...
  public beautifulWorker(callback: (dest: HTMLCanvasElement) => void): void {
    ...
    w.onmessage = (e) => {
      ...
      const d = new Uint8Array(
        ...
        destImageData.data.byteOffset,
        destImageData.data.byteLength
      );
