 ...
 * @param binary - base64 encoded sequence
 * @param start - first bit to read
 * @param len - number of bits to read
 * @return - bits from string, as number
 */
export function readBits(binary: string, start: number, len: number): number {
...
export class PackedTrie implements ITrie {
  ...
  test(str: string, opts: ITestOpts = { wildcard: null, prefix: false }) {
    ...
    return this.searchFirst(str, opts) !== null;
  }
   ...
   *                              first-only mode; otherwise return a list
   *                              of strings that match the query.
   * Return only first match that is found,
