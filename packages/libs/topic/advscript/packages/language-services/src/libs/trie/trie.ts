 ...
 * Add word to Trie
 */
type AddToTrie = (word: string | string[], trie: Trie) => Trie;
/**
 ...
 */
export function fromList(words: string[]): Trie {
  return words.reduce((trie: Trie, word) => add(word, trie), copyTrie(emptyTrie));
}
/**
 * Initiate a Trie from a list of words
 *
 ...
 * @returns the initiated Trie
 */
export function fromKeysList<T extends string>(words: T[][]): Trie {
  return words.reduce((trie: Trie, word) => add(word, trie), copyTrie(emptyTrie));
}
 ...
 * @returns the initiated Trie
 */
export const of: TrieOfWords = (...words: string[]) => {
  return words.reduce((trie: Trie, word) => add(word, trie), copyTrie(emptyTrie));
};
 ...
 */
export const add: AddToTrie = (word, root) => {
  ...
  for (const char of word) {
    ...
    if (!child) {
      child = copyTrie(emptyWithParent(node, char), node.deep + 1);
      node.end = false;
      node.children = { ...node.children, [char]: child };
    } else {
      child.num++; //有N个字符串经过它
    }
    node = child;
...
export const search: SearchPrefix = (query, trie) => {
  ...
  if (!trie.children[query[0]]) {
    return [];
  }

  return search(query.slice(1), trie.children[query[0]]);
};
...
export const has: HasWord = (word, trie) => {
  ...
  if (word.length === 0 || !trie.children[word[0]]) {
    return false;
  }

  return has(word.slice(1), trie.children[word[0]]);
};
...
export const hasPrefix: HasPrefix = (word, trie) => {
  ...
  return hasPrefix(word.slice(1), trie.children[word[0]]);
};

const copyTrie = (trie: Trie, deep = trie.deep): Trie => {
  return {
    ...trie,
    deep,
...
export function getWords(trie: Trie): string[] {
  const result: string[] = [];
...
export function findNext(root: Trie, value: string) {
