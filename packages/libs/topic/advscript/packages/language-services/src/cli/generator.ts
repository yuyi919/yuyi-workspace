...
import fs from "fs";
import { IndentNode, CompositeGeneratorNode, NL, processGeneratorNode } from "langium";
// import { State, Statemachine } from '../language-server/generated/ast';
import { extractDestinationAndName } from "./cli-util";
import path from "path";

export function generateTs<T>(
  statemachine: T,
  filePath: string,
...
export function write(ctx: GeneratorContext<any>) {
...
export function generateWithGenerator<T>(
  statemachine: T,
  filePath: string,
  destination: string | undefined,
  wrapper: (
    ...
    | [kind: "append" | "appendLine", ...texts: string[]]
    | [kind: "indent", handle: (indentNode: IndentNode) => void]
  >
): string {
  const data = extractDestinationAndName(filePath, destination);
  const ctx = {
  ...
  const generator = wrapper(ctx);
  for (const [kind, ...texts] of generator) {
    if (kind === "append") {
      ctx.fileNode.append(...texts);
    } else if (kind === "appendLine") {
      ctx.fileNode.append(...texts, NL);
    } else if (kind === "indent") {
      ctx.fileNode.indent(texts[0] as any);
    }
  }
  if (!fs.existsSync(ctx.destination)) {
    fs.mkdirSync(ctx.destination, { recursive: true });
...
export interface GeneratorContext<T> {
  statemachine: T;
  fileName: string;
  destination: string;
...
function generate<T>(ctx: GeneratorContext<T>): string {
