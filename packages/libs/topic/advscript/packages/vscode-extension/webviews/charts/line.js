...
plugin.render = function (id, datas, uipersistence, config) {
  ...
  function calculateScales(w, h) {
    y = d3
      .scaleLinear()
      .domain([min, max])
      .range([h - padding + headerHeight, padding + headerHeight]);
    x = d3
      .scaleLinear()
  ...
  vis = d3.select(id).append("svg:svg").attr("width", "100%").attr("height", height);
  ...
  var selectionRightFooter = mouseG
    .append("g")
    .attr("class", "selectionrightfooter")
    .attr("visibility", "collapse");
  var selectionLeftFooter = mouseG
    .append("g")
    .attr("class", "selectionleftfooter")
    .attr("visibility", "collapse");
  ...
  mouseGhover
    ...
    .attr("y", "2.25em")
    .style("opacity", 0.5);
  selectionRightFooter
    .append("text")
    .attr("class", "button openineditor")
    .attr("title", "Select in editor")
    .html("&#xeae9")
    .attr("y", height - footerHeight + 24)
    .on("click", function () {
  ...
  selectionRightFooter
    .append("text")
    .attr("class", "button zoom")
    .html("&#xeb81")
    .attr("y", height - footerHeight + 24)
    .attr("x", selectionRightFooterWidth)
    .on("click", zoomChart)
    .append("title")
    .text("Zoom In");
  selectionRightFooterWidth += 16; //width of button
  ...
  rightbuttons
    .append("text")
    .attr("class", "button rightbutton snap")
    .html("&#xebae")
    .attr("y", height - footerHeight + 24)
    .attr("x", width - 24)
    .append("title")
  ...
  rightbuttons
    .append("text")
    .attr("class", "button rightbutton unzoom")
    .html("&#xeb82")
    .attr("y", height - footerHeight + 24)
    .attr("x", width - 48)
    .attr("visibility", "collapse")
  ...
  rightbuttons
    .append("rect")
    .attr("class", "buttonseperator rightbutton")
    .attr("y", height - footerHeight + 24 - 14)
    .attr("x", width - 48)
    .attr("height", 14)
    .attr("width", 1)
    .attr("visibility", "collapse");
  ...
  mouseG
    .append("rect") // this is the vertical line to follow the mouse
  ...
  mouseG
    .append("rect") // this is the vertical line to follow the caret in the document
  ...
  let brush = d3
    ...
    .on("brush", function () {
      ...
      if (extent) {
        ...
        selectionRightFooter.attr(
          "transform",
          `translate(${extent[1] - rightoffset - selectionRightFooterWidth},0)`
        );
        ...
        if (selectFooterWidth > selectionWidth) {
          ...
          if (rightfooterX + selectionRightFooterWidth > maximumPosition) {
            //too far right
            rightfooterX =
              rightfooterX + (maximumPosition - (rightfooterX + selectionRightFooterWidth));
          ...
          if (rightfooterX - selectionLeftFooterWidth < minimumPosition) {
            //too far left
            rightfooterX =
              rightfooterX + (minimumPosition - (rightfooterX - selectionLeftFooterWidth));
          }
        ...
        selectionRightFooter
          .attr("transform", `translate(${rightfooterX},0)`)
          .attr("visibility", "visible");
        selectionLeftFooter
          .attr("transform", `translate(${leftfooterX},${height - footerHeight + 16})`)
          .attr("visibility", "visible");
        ...
        let pageextentstr = "";
        if (pageextent0 && pageextent1) pageextentstr = pageextent0.page + "-" + pageextent1.page;
        mouseG.select(".pageNumber").text("p." + pageextentstr);
        mouseG
          .select(".currentTime")
          .text(
            secondsToString(pageextent0 ? pageextent0.cumulativeDuration : 0) +
              "-" +
              secondsToString(pageextent1 ? pageextent1.cumulativeDuration : 0)
          );
  ...
  function doubleclick() {
    ...
    if (config.revealLine) config.revealLine(x.invert(mouse[0]));
  }
  ...
  function mousemove() {
    var mouse = d3.mouse(this);
    vis.select(".mouse-line").attr("x", mouse[0]);
  ...
  function hover(xval, xval2) {
    let values = [];
    vis.selectAll(".chart-data").each(function (d, i) {
      if (xval2) values.push([bisect(xval, i), bisect(xval2, i)]);
      else values.push(bisect(xval, i));
    });
    if (config.map) {
      ...
      if (config.map.has(lineNb)) {
        ...
        mouseG
          .select(".breadcrumbs")
          .html(
            lineinfo.sections.join(
              "<tspan class='chevron' alignment-baseline='middle'>&#xeab6</tspan>"
            )
          );
      }
    }
    ...
    structurecontainer.selectAll("rect").each(function (d, i) {
      ...
      if (transition)
        structline
          ...
          .duration(500)
          .ease(d3.easeCubic)
          .attr("x", x(structline.attr("data-line")));
      else structline.attr("x", x(structline.attr("data-line")));
    });
    structurePositions.push(width);
  }
  ...
  function positionSelection(animate) {
    ...
    if (animate)
      vis
        ...
        .duration(500)
        .attr("x", linestartX)
        .attr("width", lineendX - linestartX);
    else
      vis
        .select(".selection-box")
        .attr("x", linestartX)
