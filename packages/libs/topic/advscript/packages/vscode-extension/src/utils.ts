...
import type { StructureProperties } from "./extension";
...
export const findCharacterThatSpokeBeforeTheLast = (
  ...
  docProperties: StructureProperties
): string => {
  const isAlreadyMentionedCharacter = (text: string): boolean => docProperties.characters.has(text);
  ...
  do {
    ...
    let potentialCharacterLine = document
      .getText(new vscode.Range(beginningOfLineToInspect, endOfLineToInspect))
      .trimRight();
...
export function secondsToMinutesString(seconds: number): string {
  ...
  else return padZero(time.getHours() * 60 + time.getMinutes()) + ":" + padZero(time.getSeconds());
}

export const overwriteSceneNumbers = () => {
  telemetry.reportTelemetry("command:advscript.overwriteSceneNumbers");
...
export const updateSceneNumbers = () => {
  telemetry.reportTelemetry("command:advscript.updateSceneNumbers");
  const fullText = vscode.window.activeTextEditor.document.getText();
...
const clearSceneNumbers = (fullText: string): string => {
  ...
  const newText = fullText.replace(regexSceneHeadings, (heading: string) =>
    heading.replace(/ #.*#$/, "")
  );
  return newText;
};
...
const writeSceneNumbers = (fullText: string) => {
  ...
  const regexSceneHeadings = new RegExp(parser.regex.scene_heading.source, "igm");
  const numberingSchema = sceneNumbering.makeSceneNumberingSchema(
    sceneNumbering.SceneNumberingSchemas.Standard
  );
  let m;
  while ((m = regexSceneHeadings.exec(fullText))) {
    const matchExisting = m[0].match(/#(.+)#$/);

    if (!matchExisting) oldNumbers.push(null);
    /* no match = no number = new number required in this slot */ else if (
      numberingSchema.canParse(matchExisting[1])
    )
    ...
    vscode.window.activeTextEditor.edit((editBuilder) =>
      editBuilder.replace(
        new vscode.Range(
          ...
          new vscode.Position(vscode.window.activeTextEditor.document.lineCount, 0)
        ),
        newText
      )
    );
...
export const shiftScenes = (
  ...
  const selectSceneAt = (sel: vscode.Selection): vscode.Selection => {
    ...
    const headingsBefore = parsed.tokens
      .filter(
        (token) =>
          (token.is("scene_heading") || token.is("section")) &&
  ...
  const moveText =
    editor.document.getText(moveSelection) + new Array(numNewlinesAtEndRequired + 1).join("\n");
  ...
  const aboveSelection =
    direction == -1 &&
    selectSceneAt(
      new vscode.Selection(moveSelection.anchor.line - 1, 0, moveSelection.anchor.line - 1, 0)
    );
  const belowSelection =
    direction == 1 &&
    selectSceneAt(
      new vscode.Selection(moveSelection.active.line + 1, 0, moveSelection.active.line + 1, 0)
    );
...
export function assetsRuntimePath(...paths: string[]): string {
  return path.join(
    vscode.extensions.getExtension("yuyi919.vscode-advscript").extensionPath,
    "out",
    ...paths
  );
}
...
export function getPackageInfo(): IPackageInfo | null {
  const extension = vscode.extensions.getExtension("yuyi919.vscode-advscript");
  console.log("getPackageInfo", extension);
