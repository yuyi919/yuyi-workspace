...
export class Options {
  filepath: string;
  config: fountainconfig.CoreConfig;
  parsed: any;
...
const create_simplestream = function (filepath: string) {
  const simplestream: any = {
    ...
    write: function (chunk: any) {
      ...
      if (simplestream.filepath) {
        const fsmodule = "fs";
        const fs = require(fsmodule); // bypass requirejs minification/optimization
        const stream = fs.createWriteStream(simplestream.filepath, {
          encoding: "binary",
        });
        ...
        stream.on("error", function (err: any) {
          if (err.code == "ENOENT") {
            vscode.window.showErrorMessage(
              "Unable to export PDF! The specified location does not exist: " + err.path
            );
        ...
        stream.on("finish", () => {
          ...
          vscode.window
            .showInformationMessage("Exported PDF Succesfully!", open, reveal)
            .then((val) => {
              switch (val) {
                case open: {
...
async function initDoc(opts: Options) {
  ...
  const fp =
    __dirname.slice(0, __dirname.lastIndexOf(path.sep)) + path.sep + "courierprime" + path.sep;
  ...
  if (opts.font != "Courier Prime") {
    const variants = await fontFinder.listVariants(opts.font);
  ...
  doc.text = function (text: string, x: number, y: number, options: any) {
    ...
    const split_for_fromatting = text
      .split(/(\\\*)|(\*{1,3})|(\\?_)|(\[\[)|(\]\])/g)
      .filter(function (a) {
        return a;
      });
    ...
    for (let i = 0; i < split_for_fromatting.length; i++) {
      let elem = split_for_fromatting[i];
      if (elem === "***") {
...
const get_title_page_token = function (parsed: any, type: string): any {
  let result = null;
...
async function generate(doc: any, opts: any, lineStructs?: Map<number, lineStruct>) {
  ...
  const title_token = get_title_page_token(parsed, "title");
  let author_token = get_title_page_token(parsed, "author");
  if (!author_token) {
  ...
  const title_page_main = function (parsed?: any, type?: string, options?: any) {
    options = options || {};
    if (arguments.length === 0) {
      title_page_next_line();
      return;
    }
    const token = get_title_page_token(parsed, type);
    if (token) {
  ...
  if (cfg.print_title_page) {
    ...
    const concat_types = function (parsed: any, prev: any, type: string) {
      const token = get_title_page_token(parsed, type);
      if (token) {
    ...
    const left_side = print.title_page.left_side.reduce(concat_types.bind(null, parsed), []),
  ...
  const print_header_and_footer = function (continuation_header?: string) {
    if (cfg.print_header) {
      ...
      let offset = helpers.blank_text(continuation_header);
  ...
  lines.forEach(function (line: any) {
    if (line.type === "page_break") {
      if (cfg.scene_continuation_bottom && line.scene_split) {
        const scene_continued_text = "(" + (cfg.text_scene_continued || "CONTINUED") + ")";
        var feed =
          print.action.feed +
      ...
      if (lineStructs) {
        if (line.token.line && !lineStructs.has(line.token.line)) {
          lineStructs.set(line.token.line, {
            page: page,
            scene: currentScene,
      ...
      if (cfg.scene_continuation_top && line.scene_split) {
        ...
        let scene_continued =
          ...
          ":";
        scene_continued +=
          scene_continuations[scene_number] > 1
            ? " (" + scene_continuations[scene_number] + ")"
            : "";
      ...
      const color = (print[line.type] && print[line.type].color) || "#000000";

      const general_text_properties = {
        color: color,
        highlight: false,
      ...
      function get_text_properties(
        lline = line,
        expcfg = exportcfg,
        old_text_properties = general_text_properties
      ) {
        const new_text_properties = Object.assign({}, old_text_properties);
        if (!!expcfg && lline.type === "character") {
          let character = trimCharacterExtension(lline.text);
          // refer to Liner in ./liner.ts
          character = character.replace(/([0-9]* - )/, "");
          if (
            !!expcfg.highlighted_characters &&
            expcfg.highlighted_characters.includes(character)
          ) {
      ...
      const text_properties = get_text_properties();
      ...
      if (line.type === "centered") {
        ...
        const hasInvisibleSection =
          line.type === "scene_heading" && line.token.invisibleSections != undefined;
        function processSection(sectiontoken: any) {
          ...
          if (!hasInvisibleSection) feed += current_section_level * print.section.level_indent;
          ...
          if (cfg.create_bookmarks) {
            ...
            const oc = getOutlineChild(outline, sectiontoken.level - 1, 0);
            if (oc == undefined) console.log("BOOM");
        ...
        if (line.token && line.token.dual) {
          if (line.right_column) {
            ...
            line.right_column.forEach(function (right_line: any) {
              let feed_right = (print[right_line.type] || {}).feed || print.action.feed;
              feed_right -= (feed_right - print.left_margin) / 2;
              feed_right += (print.page_width - print.right_margin - print.left_margin) / 2;
              const right_text_properties = get_text_properties(right_line);
              doc.text(
                right_line.text,
                feed_right,
...
export const get_pdf = async function (
  opts: Options,
...
export const get_pdf_stats = async function (opts: Options): Promise<pdfstats> {
  const doc = await initDoc(opts);
  const stats: pdfstats = {
    pagecount: 1,
    pagecountReal: 1,
    linemap: new Map<number, lineStruct>(),
  };
