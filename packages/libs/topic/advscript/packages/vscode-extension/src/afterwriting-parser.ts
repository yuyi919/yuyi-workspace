...
import { getConfig as getConfig } from "./configloader";
...
export const parse = function (
  ...
  const config = getConfig(lastEditor);
    ...
    if (text.trim().length === 0 && text !== "  ") {
      const skip_separator =
        ...
        (ignoredLastToken &&
          ...
          result.tokens[result.tokens.length - 1].type == "separator");
  ...
  if (generate_html) {
    ...
    while (current_index < result.tokens.length) {
      ...
      if (
        (current_token.type == "action" || current_token.type == "centered") &&
