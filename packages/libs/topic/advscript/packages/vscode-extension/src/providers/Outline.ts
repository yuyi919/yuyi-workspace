...
import { activeDocument, activeParsedDocument, getEditor } from '../extension';
import * as config from '../configloader';

export class OutlineTreeDataProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
	...
	reveal(): void {
		const currentCursorLine = getEditor(activeDocument()).selection.active.line;
...
function makeTreeItem(token: afterparser.StructToken, parent: OutlineTreeItem): OutlineTreeItem {
	...
	const passthrough = (token.section && !config.uiPersistence.outline_visibleSections) || (!token.section && !config.uiPersistence.outline_visibleScenes);
