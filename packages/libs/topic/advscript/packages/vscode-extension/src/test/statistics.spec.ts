...
describe("Statistics", () => {
    it("Big Fish CRLF", () => {
        const bigFish = fs.readFileSync(path.resolve(__dirname, "./scripts/big_fish_crlf.advscript"), "utf-8")
        bigFishAssertions(bigFish)
    })

    it("Big Fish LF", () => {
        const bigFish = fs.readFileSync(path.resolve(__dirname, "./scripts/big_fish_lf.advscript"), "utf-8")
        bigFishAssertions(bigFish)
    })

    it("Brick & Steel CRLF", () => {
        const bigFish = fs.readFileSync(path.resolve(__dirname, "./scripts/brick_and_steel_crlf.advscript"), "utf-8")
        brickAndSteelAssertions(bigFish)
    })

    it("Brick & Steel LF", () => {
        const bigFish = fs.readFileSync(path.resolve(__dirname, "./scripts/brick_and_steel_lf.advscript"), "utf-8")
        brickAndSteelAssertions(bigFish)
    })
