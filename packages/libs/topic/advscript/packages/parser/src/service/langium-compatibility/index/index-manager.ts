...
export class DefaultIndexManager implements IndexManager {
  ...
  findAllReferences(targetNode: AstNode, astNodePath: string): Stream<ReferenceDescription> {
    ...
    this.referenceIndex.forEach((docRefs: ReferenceDescription[]) => {
      docRefs.forEach((refDescr) => {
        if (
          refDescr.targetUri.toString() === targetDocUri.toString() &&
  ...
  allElements(nodeType?: string): Stream<AstNodeDescription> {
    ...
    if (cached) {
      ...
      const elements = this.globalScopeCache
        .get("")!
        .filter((e) => (nodeType ? this.astReflection.isSubtype(e.type, nodeType) : true));
  ...
  protected isAffected(document: LangiumDocument, changed: URI): boolean {
    ...
    if (references) {
      return references
        .filter((e) => !e.local)
        .some((e) => e.targetUri.toString() === changedUriString);
    }
    return false;
  }
  ...
  protected async traverseFolder(
    folderPath: URI,
    fileExt: string[],
    documentAcceptor: (document: LangiumDocument) => void
  ): Promise<void> {
    ...
    if (this.skip(folderPath)) return;
  ...
  protected async processDocuments(
    ...
    for (const document of documents) {
      const indexData: AstNodeDescription[] =
        await this.astNodeDescriptionProvider().createDescriptions(document, cancelToken);
    ...
    for (const document of documents) {
      this.referenceIndex.set(
        ...
        await this.referenceDescriptionProvider().createDescriptions(document, cancelToken)
      );
      await interruptAndCheck(cancelToken);
