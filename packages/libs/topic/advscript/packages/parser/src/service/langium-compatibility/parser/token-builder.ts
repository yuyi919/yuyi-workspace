...
export class DefaultTokenBuilder implements TokenBuilder {
  ...
  buildTokens(grammar: Grammar): TokenType[] {
    ...
    const keywords = streamAllContents(grammar)
      .map((e) => e.node)
      .filter(isKeyword)
      .distinct((e) => e.value)
      .toArray()
  ...
  protected buildKeywordToken(
    ...
    return {
      name: keyword.value,
      PATTERN: this.buildKeywordPattern(keyword),
  ...
  protected findLongerAlt(
    ...
    for (const otherKeyword of keywords) {
      ...
      if (
        tokenType &&
        otherKeyword.value.length > keyword.value.length &&
        otherKeyword.value.startsWith(keyword.value)
      ) {
