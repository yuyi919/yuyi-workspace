...
export class DefaultFoldingRangeProvider implements FoldingRangeProvider {
  ...
  protected collectObjectFolding(
    document: LangiumDocument,
    node: AstNode,
  ...
  protected collectCommentFolding(
    document: LangiumDocument,
    node: AstNode,
    acceptor: FoldingRangeAcceptor
  ): void {
  ...
  protected toFoldingRange(
    ...
    if (!this.includeLastFoldingLine(node, kind)) {
      end = document.textDocument.positionAt(
        document.textDocument.offsetAt({ line: end.line, character: 0 }) - 1
      );
    }
