...
export class DefaultDocumentValidator {
  ...
  protected async validateAst(
    ...
    await Promise.all(
      streamAllContents(rootNode).map(async (c) => {
        await interruptAndCheck(cancelToken);
  ...
  protected toDiagnostic<N extends AstNode>(
    severity: "error" | "warning" | "info" | "hint",
    message: string,
...
export function getDiagnosticRange<N extends AstNode>(info: DiagnosticInfo<N, string>): Range {
  ...
  const cstNode =
    findNodeForFeature(info.node.$cstNode, info.property, info.index) ?? info.node.$cstNode;
  if (!cstNode) {
    return {
