...
import { EmbeddedActionsParser, IOrAlt, IToken, Lexer, TokenType } from "chevrotain";
...
import {
  ...
  LeafCstNodeImpl,
  RootCstNodeImpl,
} from "./cst-node-builder";
import { AbstractLangiumParser, ParseResult, RuleResult } from "./interface";
  ...
  action($type: string, action: Action): void {
    if (!this.wrapper.IS_RECORDING) {
      ...
      if (!last.$cstNode && action.feature && action.operator) {
        ...
        const feature = (last.$cstNode as CstNode).feature;
