...
export class StreamImpl<S, T> implements Stream<T> {
  ...
  toMap<K = T, V = T>(keyFn?: (e: T) => K, valueFn?: (e: T) => V): Map<K, V> {
    const entryStream = this.map(
      (element) => <[K, V]>[keyFn ? keyFn(element) : element, valueFn ? valueFn(element) : element]
    );
  ...
  map<U>(callbackfn: (value: T) => U): Stream<U> {
    return new StreamImpl<S, U>(this.startFn, (state) => {
  ...
  filter(predicate: (value: T) => unknown): Stream<T> {
    return new StreamImpl<S, T>(this.startFn, (state) => {
      let result: IteratorResult<T>;
      do {
  ...
  reduce<U>(
    callbackfn: (previousValue: U | T, currentValue: T) => U,
    initialValue?: U
  ): U | T | undefined {
    const iterator = this.iterator();
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const EMPTY_STREAM: Stream<any> = new StreamImpl<undefined, any>(
  () => undefined,
  () => DONE_RESULT
);
 ...
 */
export const DONE_RESULT: IteratorReturnResult<undefined> = Object.freeze({
  done: true,
  value: undefined,
});
