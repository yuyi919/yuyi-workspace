...
export function isDataTypeRule(rule: ast.ParserRule): boolean {
  ...
  const onlyRuleCallsAndKeywords = features.every(
    (e) =>
      ast.isRuleCall(e) ||
      ast.isKeyword(e) ||
      ast.isGroup(e) ||
      ast.isAlternatives(e) ||
      ast.isUnorderedGroup(e)
  );
  if (onlyRuleCallsAndKeywords) {
    const ruleCallWithParserRule = features.filter(
      (e) => ast.isRuleCall(e) && ast.isParserRule(e.rule.ref) && !isDataTypeRule(e.rule.ref)
    );
...
export function findAllFeatures(rule: RuleWithAlternatives): {
 ...
 */
function findNodesForFeatureInternal(
  node: CstNode | undefined,
  feature: string | undefined,
...
function terminalAlternativesToRegex(alternatives: ast.TerminalAlternatives): string {
  return withCardinality(
    `(${alternatives.elements.map(abstractElementToRegex).join("|")})`,
...
export function computeGrammarScope(
