...
export class DefaultCompletionProvider implements CompletionProvider {
  ...
  protected completionForCrossReference(
    ...
    if (assignment && parserRule) {
      const scope = this.scopeProvider.getScope(
        context,
        `${getTypeNameAtElement(parserRule, assignment)}:${assignment.feature}`
      );
  ...
  protected completionForKeyword(
    keyword: ast.Keyword,
    context: AstNode | undefined,
    acceptor: CompletionAcceptor
  ): void {
    acceptor(keyword.value, {
  ...
  protected findCommonSuperRule(
    node: CstNode
  ): { rule: ast.ParserRule; node: CstNode } | undefined {
  ...
  protected fillCompletionItem(
    ...
    offset: number,
    value: string | AstNode | AstNodeDescription,
    info: Partial<CompletionItem> | undefined
  ...
  protected buildCompletionTextEdit(
    ...
    for (let i = completionLowerCase.length; i > 0; i--) {
      ...
      if (
        ...
        (i === 0 || !this.isWordCharacterAt(contentLowerCase, offset - i - 1))
      ) {
        negativeOffset = i;
        break;
      }
