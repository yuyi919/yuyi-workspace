...
type LangiumGrammarChecks = {
  [type in ast.LangiumGrammarAstType]?: ValidationCheck | ValidationCheck[];
};
...
export class LangiumGrammarValidationRegistry extends ValidationRegistry {
  constructor(services: LangiumGrammarServices) {
    ...
    const checks: LangiumGrammarChecks = {
      ...
      ParserRule: [validator.checkParserRuleDataType],
      TerminalRule: [
...
export class LangiumGrammarValidator {
    ...
    if (grammar.name) {
      ...
      if (firstChar.toUpperCase() !== firstChar) {
        accept("warning", "Grammar name should start with an upper case letter.", {
          node: grammar,
          property: "name",
          code: IssueCodes.GrammarNameUppercase,
        });
      }
    }
  }
  ...
  checkFirstGrammarRule(grammar: ast.Grammar, accept: ValidationAcceptor): void {
    ...
    if (firstRule) {
      if (isDataTypeRule(firstRule)) {
        accept("error", "The entry rule cannot be a data type rule.", {
          node: firstRule,
          property: "name",
        });
      } else if (firstRule.fragment) {
        accept("error", "The entry rule cannot be a fragment.", {
          node: firstRule,
          property: "name",
        });
      }
    } else {
      accept("error", "This grammar is missing an entry parser rule.", {
        node: grammar,
        property: "name",
      });
    }
  ...
  checkGrammarHiddenTokens(grammar: ast.Grammar, accept: ValidationAcceptor): void {
    if (grammar.definesHiddenTokens) {
      accept("error", "Hidden terminals are declared at the terminal definition.", {
        ...
        code: IssueCodes.HiddenGrammarTokens,
      });
    }
  }
  ...
  checkHiddenTerminalRule(terminalRule: ast.TerminalRule, accept: ValidationAcceptor): void {
    if (terminalRule.hidden && terminalRule.fragment) {
      accept("error", "Cannot use terminal fragments as hidden tokens.", {
        node: terminalRule,
        property: "hidden",
      });
    }
  ...
  checkEmptyTerminalRule(terminalRule: ast.TerminalRule, accept: ValidationAcceptor): void {
    try {
      ...
      if (new RegExp(regex).test("")) {
        accept("error", "This terminal could match an empty string.", {
          node: terminalRule,
          property: "name",
        });
      }
  ...
  checkUsedHiddenTerminalRule(
    ruleCall: ast.RuleCall | ast.TerminalRuleCall,
    accept: ValidationAcceptor
  ): void {
    const parentRule = getContainerOfType(
      ruleCall,
      (n): n is ast.TerminalRule | ast.ParserRule => ast.isTerminalRule(n) || ast.isParserRule(n)
    );
    if (parentRule) {
      ...
      if (ast.isTerminalRule(ref) && ref.hidden) {
        accept("error", "Cannot use hidden terminal in non-hidden rule", {
          node: ruleCall,
          property: "rule",
        });
      }
    ...
    if (range.right) {
      ...
      if (!invalid) {
        accept("hint", "Consider using regex instead of character ranges", {
          node: range,
          code: IssueCodes.UseRegexTokens,
        });
      }
    }
  }
  ...
  checkTerminalRuleReturnType(rule: ast.TerminalRule, accept: ValidationAcceptor): void {
    if (rule.type && !isPrimitiveType(rule.type)) {
      accept(
        "error",
        "Terminal rules can only return primitive types like 'string', 'boolean', 'number' or 'date'.",
        { node: rule, property: "type" }
      );
    }
  }
