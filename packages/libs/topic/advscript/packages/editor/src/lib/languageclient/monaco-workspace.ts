...
export class MonacoWorkspace implements Workspace {
  ...
  public applyEdit(workspaceEdit: WorkspaceEdit): Promise<boolean> {
    ...
    const models: { [uri: string]: monaco.editor.IModel } = edit.edits
      ? edit.edits.reduce((acc: { [uri: string]: monaco.editor.IModel }, currentEdit) => {
    ...
    const editsByResource: { [uri: string]: monaco.editor.IIdentifiedSingleEditOperation[] } =
      edit.edits
        ? edit.edits.reduce(
            (
              acc: { [uri: string]: monaco.editor.IIdentifiedSingleEditOperation[] },
              currentEdit
            ) => {
              const textEdit = currentEdit as monaco.languages.WorkspaceTextEdit;
