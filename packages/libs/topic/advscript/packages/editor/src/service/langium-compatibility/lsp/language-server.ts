...
export function addDocumentsHandler(
  ...
  connection.onDidChangeWatchedFiles(async (params) => {
    const changedUris = params.changes
      .filter((e) => e.type !== FileChangeType.Deleted)
      .map((e) => URI.parse(e.uri));
    const deletedUris = params.changes
      .filter((e) => e.type === FileChangeType.Deleted)
      .map((e) => URI.parse(e.uri));
...
export function addCompletionHandler(connection: Connection, services: LangiumServices): void {
  ...
  connection.onCompletion(
    createHandler((document, params, cancelToken) => {
...
export function addGotoDefinitionHandler(connection: Connection, services: LangiumServices): void {
  connection.onDefinition(
    createHandler(
      (document, params, cancelToken) =>
        services.lsp.GoToResolver.goToDefinition(document, params, cancelToken),
      services
    )
  );
}

export function addDocumentHighlightsHandler(
  connection: Connection,
  services: LangiumServices
): void {
...
export function addHoverHandler(connection: Connection, services: LangiumServices): void {
  ...
  connection.onHover(
    createHandler(
      (document, params, cancelToken) =>
        hoverProvider.getHoverContent(document, params, cancelToken),
      services
    )
  );
}
...
export function addFoldingRangeHandler(connection: Connection, services: LangiumServices): void {
  ...
  connection.onFoldingRanges(
    createHandler(
      (document, params, cancelToken) =>
        foldingRangeProvider.getFoldingRanges(document, params, cancelToken),
      services
    )
  );
}
