...
): (input: string) => Promise<BuildResult<T>> {
...
export function expectSymbols(
  services: LangiumServices,
  expectEqual: ExpectFunction
): (input: ExpectedSymbols) => Promise<void> {
  return async (input) => {
...
export function expectFoldings(
  services: LangiumServices,
  expectEqual: ExpectFunction
): (input: ExpectedBase) => Promise<void> {
  return async (input) => {
    ...
    const foldings = await foldingRangeProvider.getFoldingRanges(
      document,
...
export function expectCompletion(
  services: LangiumServices,
  expectEqual: ExpectFunction
): (completion: ExpectedCompletion) => Promise<void> {
  return async (expectedCompletion) => {
    ...
    const items = completions.items.sort(
      (a, b) => a.sortText?.localeCompare(b.sortText || "0") || 0
    );
...
export function expectGoToDefinition(
  ...
  return async (expectedGoToDefinition) => {
    ...
    const locationLink =
      (await goToResolver.goToDefinition(
        document,
...
export function expectHover(
  services: LangiumServices,
  cb: ExpectFunction
): (expectedHover: ExpectedHover) => Promise<void> {
  return async (expectedHover) => {
    ...
    const hoverContent =
      hover && MarkupContent.is(hover.contents) ? hover.contents.value : undefined;
...
function replaceIndices(base: ExpectedBase): {
  output: string;
  indices: number[];
