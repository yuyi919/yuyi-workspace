...
`,
    ...
    function applyEdits(text: string, range = getSelectionRange(), name = "invoke") {
      // const id = { major: 1, minor: 1 };
      // const op = { identifier: id, range: range, text: text, forceMoveMarkers: true };
      // editor.executeEdits(name, [op]);
      editor
        .getModel()
        .pushEditOperations([], [{ range: range, text: text, forceMoveMarkers: true }], () => {
          return [];
        });
    }
    globalThis.applyEdits = applyEdits;

    editor.addCommand(monaco.KeyMod.Alt + monaco.KeyCode.Enter, (...args) => {
      //@ts-ignore
      // editor.invokeWithinContext((a) =>
      //   EditorExtensionsRegistry.getEditorActions()
      //     .find((o) => o.id === "editor.action.triggerSuggest")
      //     .runEditorCommand(a, editor, { auto: true })
      // );
      // // editor.trigger("", "editor.action.triggerSuggest", { mode: "Invoked" });
      // console.log(args);
      triggerCommand(COMMAND_ID.TriggerSuggest);
    });
    // monaco.editor.registerCommand("editor.action.triggerSuggest", (e, ...args) => {
    //   console.log(e);
    //   EditorExtensionsRegistry.getEditorActions()
    //     .find((o) => o.id === "editor.action.triggerSuggest")
    //     .runEditorCommand(e, editor, ...args);
    // });
    // .addAction(monaco.KeyMod.Alt + monaco.KeyCode.Enter, (e) => {
    //   //@ts-ignore
    //   editor.invokeWithinContext((a) =>
    //     EditorExtensionsRegistry.getEditorActions()
    //       .find((o) => o.id === "editor.action.triggerSuggest")
    ...
    // });
    // editor;
    // editor.getModel().getact(source, handlerId, payload)
